...
concurrent_object allocate_object(int initial_data,int readers_limit ,int array_idx){
    concurrent_object object;
    object.data = initial_data;
    object.readers_count = 0;
    object.array_idx = array_idx;
    object.readers_limit = readers_limit;
...
bool read_from_object(concurrent_object* object){
    ...
    if(object->readers_count == object->readers_limit){
        pthread_mutex_unlock(&object->readers_mutex);
        return false;
    }
...
int main(){
    ...
    for(int i = 0; i<num_objects; i++){
        objects[i] = allocate_object(initial_data, i, 1 + rand() % readers_limit_max);
    }
